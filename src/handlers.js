...
async function getFavourites() {
    // return await fetch('http://localhost:3000/weather/favourites').then(r => r.json());
    return await fetch(`${herokuURL}/favourites`).then(r => r.json());
}

async function addCityToDatabase(name) {
    // return await fetch(`http://localhost:3000/weather/addCity?name=${name}`, {method: "PUT"});
    return await fetch(`${herokuURL}/addCity?name=${name}`, {method: "PUT"});
}
